...
+ (instancetype)showInputViewInView:(UIView*)view itemSelfType:(XJItemSelType)itemSelfType contentTextView:(UITextView*)contentTextView;
{
    ...
    switch (itemSelfType) {
        ...
        {
            itemSelfView.xj_top = XJScreenHeight - InputHight;
...
- (void) adjustPublishToolsViewFrame:(CGRect)rect publishSelType:(publishSelType)type;
{
    ...
    switch (self.itemSelfType) {
        case XJItemSelTypeInput:{
             self.xj_top =  rect.origin.y - self.inputView.xj_height;
        }
            break;
        case XJItemSelTypeMulitfuntion:{
             self.xj_top =  rect.origin.y - showSumHeight;
        }
            break;
        case XJItemSelTypeChat:{
             self.xj_top =  rect.origin.y - self.chatInputView.xj_height;
        }
            break;
    ...
    switch (type) {
        ...
        {
            self.faceEmojeView.xj_top = XJScreenHeight - FaceEmojeViewHeight;
        ...
        {
            self.faceEmojeView.xj_top = XJScreenHeight;
            self.toobarView.xj_top = XJScreenHeight - FaceEmojeViewHeight;
        }
            break;
        default:
            break;
...
{
    if (!_toobarView) {
        _toobarView = [[XJToolBarView alloc]initWithFrame:CGRectMake(0, XJScreenHeight, XJScreenWidth, FaceEmojeViewHeight)];
